...
CClassMenuPanel::CClassMenuPanel(int iTrans, int iRemoveMe, int x,int y,int wide,int tall) : CMenuPanel(iTrans, iRemoveMe, x,y,wide,tall)
{
	...
	for (int i = 0; i <= PC_RANDOM; i++)
	{
		...
		// Class button
		snprintf(sz, 256, "%s", CHudTextMessage::BufferedLocaliseTextString( sLocalisedClasses[i] ) );
		...
		snprintf(sz, 256, "#Title_%s", sTFClassSelection[i]);
		...
		if ( bShowClassGraphic )
		{
			for ( int team = 0; team < 2; team++ )
			{
				if ( team == 1 )
				{
					snprintf( sz, 256, "%sred", sTFClassSelection[i] );
				}
				else
				{
					snprintf( sz, 256, "%sblue", sTFClassSelection[i] );
				}
		...
		snprintf(sz, 256, "classes/short_%s.txt", sTFClassSelection[i]);
...
void CClassMenuPanel::Update()
{
	...
	for (int i = 0; i <= PC_RANDOM; i++)
	{
		...
		char sz[256]; 
		snprintf(sz, 256, m_sPlayersOnTeamString, iTotal);
