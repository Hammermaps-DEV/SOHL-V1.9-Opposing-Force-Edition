...
DWORD WINAPI WatchThread(LPVOID lpParam)
{
	...
	while (1)
	{
		...
		char *list[] = { "Half-Life", "SoHL: Opposing-Force", "Xash3D Launcher","Xash3D Launcher (32 bit)", NULL };
		for (int i = 0; list[i] != '\0'; i++) {
			if (list[i] != NULL) {
				g_hWnd = FindWindow(NULL, list[i]);
				if (g_hWnd) {
					break;
				}
			} else {
...
void CSoundEngine::Init(void)
{
	...
	if (m_iVersion != FMOD_VERSION)
		...
		ENGINEPRINT("FMOD Version '%08x' is initialized.\n", FMOD_VERSION);
...
====================
*/
void CSoundEngine::PlayMusic(char *pszFile, int flags)
{
	if (m_bPlayingMusic) {
	...
	_FMOD_System_CreateStream(m_pSystem, szPath, flags, NULL, &m_pMusicSound);
