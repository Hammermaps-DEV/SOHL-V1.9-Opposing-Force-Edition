...
#define MAX_STUCKTABLE_ENTRIES	54
...
int FindTextureLinear(const char* const pszName)
{
	...
	{
		if (_stricmp(m_szTextureName[i], pszName) == 0)
		{
			return i;
		}
	}
...
void PM_InitTextureTypes(char *filename)
{
	...
	byte *pMemFile = pmove->COM_LoadFile(filename, 5, NULL);
	...
	while (pmove->memfgets(pMemFile, fileSize, &filePos, buffer, sizeof(buffer)) && (gcTextures < CTEXTURESMAX))
	{
		...
		int iFound = FindTextureLinear(&(buffer[i]));
		...
		else
		{
			...
			case CHAR_TEX_DEEP_SNOW:
				...
				if (pmove->flags & FL_DUCKING) {
					pmove->clientmaxspeed = 150;
				}
				else {
					pmove->clientmaxspeed = 200;
				}
				break;
			case CHAR_TEX_GRASS:
				fvol = fWalking ? 0.2f : 0.5f;
