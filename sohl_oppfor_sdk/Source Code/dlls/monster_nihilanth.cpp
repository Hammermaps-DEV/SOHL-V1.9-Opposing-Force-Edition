...
void CNihilanth::PainSound(void)
{
	...
	{
		EMIT_SOUND(edict(), CHAN_VOICE, RANDOM_SOUND_ARRAY(pLaughSounds), VOL_NORM, 0.2);
	}
	else if (m_irritation >= 2)
	{
		EMIT_SOUND(edict(), CHAN_VOICE, RANDOM_SOUND_ARRAY(pPainSounds), VOL_NORM, 0.2);
	}
}

void CNihilanth::DeathSound(void)
{
	EMIT_SOUND(edict(), CHAN_VOICE, RANDOM_SOUND_ARRAY(pDeathSounds), VOL_NORM, 0.1);
}
...
void CNihilanth::NextActivity()
{
	...
	if ((pev->health < gSkillData.nihilanthHealth / 2 || m_iActiveSpheres < N_SPHERES / 2) && m_hRecharger == NULL && m_iLevel <= 9)
	{
		...
		snprintf(szName, 64, "%s%d", m_szRechargerTarget, m_iLevel);
	...
	if (m_hRecharger != NULL)
	{
		...
		if (flDist < 128.0)
		{
			...
			if (iseq != pev->sequence)
			{
				...
				snprintf(szText, 64, "%s%d", m_szDrawUse, m_iLevel);
		...
		if (m_flLastSeen + 5 > UTIL_GlobalTimeBase() && flDist < 256 && flDot > 0)
		{
			...
			else
			{
				...
				else
				{
					...
					snprintf(szText, 64, "%s%d", m_szTeleportTouch, m_iTeleport);
					CBaseEntity *pTouch = UTIL_FindEntityByTargetname(NULL, szText);

					snprintf(szText, 64, "%s%d", m_szTeleportUse, m_iTeleport);
...
void CNihilanth::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	case 2:	// zen
		if (m_hEnemy != NULL)
		{
			if (RANDOM_LONG(0, 4) == 0)
				EMIT_SOUND(edict(), CHAN_VOICE, RANDOM_SOUND_ARRAY(pAttackSounds), VOL_NORM, 0.2);

			EMIT_SOUND(edict(), CHAN_WEAPON, RANDOM_SOUND_ARRAY(pBallSounds), VOL_NORM, 0.2);
	...
	case 3:	// prayer
		if (m_hEnemy != NULL)
		{
			...
			snprintf(szText, 32, "%s%d", m_szTeleportTouch, m_iTeleport);
			CBaseEntity *pTouch = UTIL_FindEntityByTargetname(NULL, szText);

			snprintf(szText, 32, "%s%d", m_szTeleportUse, m_iTeleport);
			CBaseEntity *pTrigger = UTIL_FindEntityByTargetname(NULL, szText);
	...
	{
		EMIT_SOUND(edict(), CHAN_VOICE, RANDOM_SOUND_ARRAY(pRechargeSounds), VOL_NORM, 0.2);
	}
	break;
	case 6:
		if (m_hEnemy != NULL)
...
void CNihilanthHVR::ZapInit(CBaseEntity *pEnemy)
{
	...
	EMIT_SOUND_DYN(edict(), CHAN_WEAPON, "debris/zap4.wav", VOL_NORM, ATTN_NORM, 0, 100);
}
...
void CNihilanthHVR::TeleportInit(CNihilanth *pOwner, CBaseEntity *pEnemy, CBaseEntity *pTarget, CBaseEntity *pTouch)
{
	...
	EMIT_SOUND_DYN(edict(), CHAN_WEAPON, "x/x_teleattack1.wav", VOL_NORM, 0.2, 0, 100);
}
