...
#define HEAD_M203					3
#define HEAD_M203_BLACK				4
#define HEAD_MP						5
#define HEAD_MP5					6
#define HEAD_MP5_BLACK				7
...
const char *CHGrunt::pAttackSoundsMP5[] = {
	"hgrunt/gr_mgun1.wav",
	"hgrunt/gr_mgun2.wav"
};
...
TYPEDESCRIPTION	CHGrunt::m_SaveData[] = {
	...
	DEFINE_FIELD(CHGrunt, m_iszSpeakAs, FIELD_STRING), //LRC
};
...
BOOL CHGrunt::CheckRangeAttack1(float flDot, float flDist) {
	if (!HasConditions(bits_COND_ENEMY_OCCLUDED) && flDist <= 2048 && flDot >= 0.5
		&& NoFriendlyFire() && m_fLockShootTime < UTIL_GlobalTimeBase()) {
		TraceResult	tr;
...
void CHGrunt::ShootMP5(void) {
	...
	if (m_cAmmoLoaded > 0) {
		...
		EMIT_SOUND_ARRAY_DYN(CHAN_WEAPON, pAttackSoundsMP5);
		...
		EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/dryfire1.wav", VOL_NORM, ATTN_NORM);
	}
...
// Shoot Shotgun
//=========================================================
void CHGrunt::ShootShotgun(void) {
	if ((m_hEnemy == NULL && m_pCine == NULL) || !NoFriendlyFire()) {
		return;
	}
...
void CHGrunt::ShootShotgunDouble(void) {
	if ((m_hEnemy == NULL && m_pCine == NULL) || !NoFriendlyFire()) {
		return;
	}
...
void CHGrunt::Precache() {
	...
	PRECACHE_SOUND("hgrunt/gr_reload1.wav"); //Reload

	PRECACHE_SOUND("weapons/glauncher.wav"); //MP5 Grenate
	...
	PRECACHE_SOUND_ARRAY(pAttackSoundsMP5);
