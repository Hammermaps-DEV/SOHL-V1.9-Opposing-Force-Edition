...
void CBigMomma::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	case BIG_AE_MELEE_ATTACK1:
	{
		...
		if (pHurt)
		{
			...
			EMIT_SOUND_DYN(edict(), CHAN_WEAPON, RANDOM_SOUND_ARRAY(pAttackHitSounds), VOL_NORM, ATTN_NORM, 0, 100 + RANDOM_LONG(-5, 5));
		}
	}
	break;
...
void CBigMomma::LayHeadcrab()
{
	...
	EMIT_SOUND_DYN(edict(), CHAN_WEAPON, RANDOM_SOUND_ARRAY(pBirthSounds), VOL_NORM, ATTN_NORM, 0, 100 + RANDOM_LONG(-5, 5));
	m_crabCount++;
}
...
void CBigMomma::LaunchMortar()
{
	...
	EMIT_SOUND_DYN(edict(), CHAN_WEAPON, RANDOM_SOUND_ARRAY(pSackSounds), VOL_NORM, ATTN_NORM, 0, 100 + RANDOM_LONG(-5, 5));
...
void CBigMomma::NodeStart(int iszNextNode)
{
	...
	if (pev->netname)
	{
		...
		if (!FNullEnt(pentTarget))
			pTarget = CBaseEntity::Instance(pentTarget);
	}
...
void CBigMomma::StartTask(Task_t *pTask)
{
	...
	case TASK_MELEE_ATTACK1:
		...
		EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, RANDOM_SOUND_ARRAY(pAttackSounds), VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
