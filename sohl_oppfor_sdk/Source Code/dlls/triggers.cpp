...
void CTrainSetSpeed::UpdateSpeed()
{
	if (!m_pTrain || !m_pTrain->pev->speed)
		return;	// no train or train not moving
...
public:
	...
	void Precache();
	void Think();
	int ObjectCaps() { return CBaseDelay::ObjectCaps() | FCAP_FORCE_TRANSITION; }	// Always go across transitions
};
...
public:
	...
	void	Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value);
	void	Think();
	STATE	GetState() { return m_hActivator == NULL ? STATE_OFF : STATE_ON; }
