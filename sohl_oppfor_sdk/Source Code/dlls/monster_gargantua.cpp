...
void CStomp::Spawn()
{
	...
	pev->renderamt = 0;
	EMIT_SOUND_DYN(edict(), CHAN_BODY, GARG_STOMP_BUZZ_SOUND, VOL_NORM, ATTN_NORM, 0, PITCH_NORM * 0.55);
}
...
void CGargantua::StompAttack()
{
	...
	EMIT_SOUND_DYN(edict(), CHAN_WEAPON, pStompSounds[RANDOM_LONG(0, HL_ARRAYSIZE(pStompSounds) - 1)], VOL_NORM, ATTN_GARG, 0, PITCH_NORM + RANDOM_LONG(-10, 10));
...
void CGargantua::TraceAttack(entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	if (bitsDamageType & (GARG_DAMAGE | DMG_BLAST))
	{
		...
		{
			EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, pPainSounds[RANDOM_LONG(0, HL_ARRAYSIZE(pPainSounds) - 1)], VOL_NORM, ATTN_GARG, 0, PITCH_NORM);
...
void CGargantua::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	case GARG_AE_SLASH_LEFT:
	{
		...
		if (pHurt)
		{
			...
			EMIT_SOUND_DYN(edict(), CHAN_WEAPON, pAttackHitSounds[RANDOM_LONG(0, HL_ARRAYSIZE(pAttackHitSounds) - 1)], VOL_NORM, ATTN_NORM, 0, 50 + RANDOM_LONG(0, 15));
		}
		else // Play a random attack miss sound
			EMIT_SOUND_DYN(edict(), CHAN_WEAPON, pAttackMissSounds[RANDOM_LONG(0, HL_ARRAYSIZE(pAttackMissSounds) - 1)], VOL_NORM, ATTN_NORM, 0, 50 + RANDOM_LONG(0, 15));
	...
	case GARG_AE_LEFT_FOOT:
		...
		EMIT_SOUND_DYN(edict(), CHAN_BODY, pFootSounds[RANDOM_LONG(0, HL_ARRAYSIZE(pFootSounds) - 1)], VOL_NORM, ATTN_GARG, 0, PITCH_NORM + RANDOM_LONG(-10, 10));
		break;
	...
	case GARG_AE_BREATHE:
		if (!(pev->spawnflags & SF_MONSTER_SPAWNFLAG_2) || m_MonsterState != MONSTERSTATE_IDLE)
			EMIT_SOUND_DYN(edict(), CHAN_VOICE, pBreatheSounds[RANDOM_LONG(0, HL_ARRAYSIZE(pBreatheSounds) - 1)], VOL_NORM, ATTN_GARG, 0, PITCH_NORM + RANDOM_LONG(-10, 10));
		break;
...
void CGargantua::StartTask(Task_t *pTask)
{
	...
	case TASK_SOUND_ATTACK:
		if (RANDOM_LONG(0, 100) < 30)
			EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, pAttackSounds[RANDOM_LONG(0, HL_ARRAYSIZE(pAttackSounds) - 1)], VOL_NORM, ATTN_GARG, 0, PITCH_NORM);
		TaskComplete();
		break;
