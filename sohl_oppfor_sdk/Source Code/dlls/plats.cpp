...
public:
	...
	STATE	GetState() { return (m_pTrain || m_pTrackTrain) ? STATE_ON : STATE_OFF; }
...
void CFuncTrackTrain::ClearPointers()
{
...
void CFuncTrackTrain::DesiredAction() // Next( void )
{
	float time = 0.5;
					...
					case DIRECTION_DESTINATION:
						if (m_pDestination)
						{
							...
							while (iDir == DIRECTION_NONE)
							{
								if (pCurForward)
								{
									fForward += Vector((pCurForward->pev->origin - vecFTemp)).Length();
									vecFTemp = pCurForward->pev->origin;
								...
								if (pCurBackward)
								{
									fBackward += Vector((pCurBackward->pev->origin - vecBTemp)).Length();
									vecBTemp = pCurBackward->pev->origin;
