...
void CPitDroneSpit::Touch(CBaseEntity *pOther)
{
	...
	else
	{
		...
		pOther->TraceAttack(pevOwner, 1 /*gSkillData.pitdroneDmgSpit*/, pev->velocity.Normalize(), &tr, DMG_GENERIC | DMG_NEVERGIB);
...
void CPitDrone::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	{
		CBaseEntity *pHurt = CheckTraceHullAttack(70, 1 /*gSkillData.pitdroneDmgWhip*/, DMG_SLASH);
		if (pHurt)
		{
	...
	{
		CBaseEntity *pHurt = CheckTraceHullAttack(70, 1 /*gSkillData.pitdroneDmgBite*/, DMG_SLASH);
		if (pHurt)
		{
			if (pHurt->pev->flags & (FL_MONSTER | FL_CLIENT))
			{
...
void CPitDrone::Spawn()
{
	...
	pev->effects = 0;
	pev->health = 20;/*gSkillData.pitdroneHealth;*/
