...
int CRope::Save( CSave& save )
{
	...
	return save.WriteFields( "CRope", this, m_SaveData, HL_ARRAYSIZE( m_SaveData ) );
}
...
int CRope::Restore( CRestore& restore )
{
	...
	auto status = restore.ReadFields( "CRope", this, m_SaveData, HL_ARRAYSIZE( m_SaveData ) );
	...
	if (uiNumSprings > 0)
	{
		...
		for (size_t uiIndex = 0; uiIndex < m_SpringCnt; ++uiIndex)
		{
			...
			if (uiIndex < m_uiSegments)
			{
				...
				spring.restLength = Vector((pSegment->pev->origin - vecOrigin)).Length();
			}
			else
				spring.restLength = 0;
		...
		while (true)
		{
			...
			if (flOldDist > m_flAttachedObjectsOffset)
			{
				...
				if (m_uiAttachedObjectsSegment < m_uiSegments)
				{
					...
					flNewOffset = Vector((pSegment->pev->origin - vecOrigin)).Length();
				}
		...
		if (flDistance > 0.0)
		{
			...
			while (bOnRope)
			{
				if (m_uiAttachedObjectsSegment < m_uiSegments)
				{
					...
					flSegLength = Vector((pSegment->pev->origin - vecOrigin)).Length();
				}
...
void CRope::Creak()
{
	EMIT_SOUND(edict(), CHAN_BODY,
		g_pszCreakSounds[RANDOM_LONG(0, HL_ARRAYSIZE(g_pszCreakSounds) - 1)],
		VOL_NORM, ATTN_NORM);
}
...
float CRope::GetSegmentLength(size_t uiSegmentIndex) const
{
	if (uiSegmentIndex < m_uiSegments)
	{
		...
		return Vector((pSegment->pev->origin - vecOrigin)).Length();
	}
...
float CRope::GetRopeLength() const
{
	...
	for (size_t uiIndex = 0; uiIndex < m_uiSegments; ++uiIndex)
	{
		...
		flLength += Vector((pSegment->pev->origin - vecOrigin)).Length();
	}
